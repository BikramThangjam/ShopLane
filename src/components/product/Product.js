...
const Product = (props) => {
    ...
    const favourites = useSelector(favSelector).favs;
    const dispatch = useDispatch();

    const itemIndex = carts.findIndex((item) => item.id === product.id);
    const favItemIndex = favourites.findIndex((item)=>item.id === product.id);
        ...
        <>
            <div className="card h-100 card-main">
                <div className="d-flex justify-content-end">
                    {
                        favItemIndex >=0 
                        ? ( <i className="fa fa-heart heart-icon red" onClick={(e) => handleFav(e, product)}></i>)
